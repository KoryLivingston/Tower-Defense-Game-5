...
Public Class Form1
    ...
    'Arrays of Enemy and picturebox for each enemy
    ...
    'LastEnemy holds the last enemy object to be spawned, used to check if this enemy has died and to then the next group of enemies for that wave will be spawned 2 seconds later, if needed

    Private additionalEnemies As Integer
    Private EnemyNum As Integer
    Private LastEnemy As Enemy

    'Arrays of Tower and picturebox for each tower
    'List to hold the objects of all towers that are created as well as to be used in the code which makes use of the objects
    'List is used to delete towers that have been sold

    Private currentTowers As New List(Of Tower)
    Private Towers(-1) As Tower
    Private PicTower(-1) As PictureBox

    'Arrays of Panel, Label and pictureboxe for the UI for each tower

    Private TowerUI(-1) As Panel
    Private LblBuffPrice(-1) As Label
    Private UpgradeSlots1(-1) As PictureBox
    Private UpgradeSlots2(-1) As PictureBox
    Private UpgradeSlots3(-1) As PictureBox


    'Towercount is used to redclare the arrays for the towers to hold the new position of the tower to be created
    ...
    Public Sub WaveSpawn()
        ...
        'Sets the lastenemy to the last enemy to be brought into the game to eb used in the condition to bring the next group of enemies in
    ...
    Private Sub EnemySpawnTimer_Tick(sender As Object, e As EventArgs) Handles NextSpawnDelay.Tick
        ...
        'If the timer has not been stopped then totalenemiesinwave must be greater than 20, therefore enemiesleftover is set to true
    ...
    .Location = PointToClient(Cursor.Position) - p
            ...
            'Adds the picturebox as handler to the click event, therefore if the newly created picturebox is clicket it will run the sub routine

            AddHandler PicTower(TowerCount).Click, AddressOf Tower_Click


            'Remain redelcarations and creations of panels, labels and pictureboxes are for every tower objects UI
